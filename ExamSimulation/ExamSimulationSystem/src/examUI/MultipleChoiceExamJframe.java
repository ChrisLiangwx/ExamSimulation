				...
				RadioButton_A.addActionListener(new ActionListener(){
					public void actionPerformed(ActionEvent e){
						...
						if(!MultipleChoiceExamJframe.isExist(User.qbName+User.userID+String.valueOf(j))) {//不存在，插入新数据
							...
							String[] str=new String[]{String.valueOf(j), User.qbName+User.userID+String.valueOf(j), stem, A, B, C, D, answer,MyAnswer};
				...
				RadioButton_A.addActionListener(new ActionListener(){
					public void actionPerformed(ActionEvent e){
						...
						if(!MultipleChoiceExamJframe.isExist(User.qbName+User.userID+String.valueOf(array[j-1]))) {//不存在，插入新数据
							String sql="insert into result (QuestionNo,QuestionID,QuestionStem,A,B,C,D,Answer,MyAnswer) values(?,?,?,?,?,?,?,?,?)";
							String[] str=new String[]{String.valueOf(j), User.qbName+User.userID+String.valueOf(array[j-1]), stem, A, B, C, D, answer,MyAnswer};
							exam.AddOrUpdate(sql, str);
							System.out.println("第"+j+"题选："+MyAnswer);
						}
						else {//已存在，更改数据
