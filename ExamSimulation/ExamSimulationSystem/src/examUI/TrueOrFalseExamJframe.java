			...
			for(int i = 1 ; i <= QuestionNo ; i++) {
				...
				RadioButton_T.addActionListener(new ActionListener(){
					public void actionPerformed(ActionEvent e){
						...
						if(!TrueOrFalseExamJframe.isExist(User.qbName+User.userID+String.valueOf(j))) {//不存在，插入新数据
							String sql="insert into result (QuestionNo,QuestionID,QuestionStem,Answer,MyAnswer) values(?,?,?,?,?)";
							String[] str=new String[]{String.valueOf(j), User.qbName+User.userID+String.valueOf(j), stem, answer,"对"};
				...
				RadioButton_F.addActionListener(new ActionListener(){
					public void actionPerformed(ActionEvent e){
						...
						if(!TrueOrFalseExamJframe.isExist(User.qbName+User.userID+String.valueOf(j))) {//不存在，插入新数据
							String sql="insert into result (QuestionNo,QuestionID,QuestionStem,Answer,MyAnswer) values(?,?,?,?,?)";
							String[] str=new String[]{String.valueOf(j), User.qbName+User.userID+String.valueOf(j), stem, answer,"错"};
							exam.AddOrUpdate(sql, str);
			...
			for(int i = 0 ; i < QuestionNo ; i++) {
				...
				RadioButton_T.addActionListener(new ActionListener(){
					public void actionPerformed(ActionEvent e){
						...
						if(!TrueOrFalseExamJframe.isExist(User.qbName+User.userID+String.valueOf(array[j-1]))) {//不存在，插入新数据
							String sql="insert into result (QuestionNo,QuestionID,QuestionStem,Answer,MyAnswer) values(?,?,?,?,?)";
							String[] str=new String[]{String.valueOf(j), User.qbName+User.userID+String.valueOf(array[j-1]), stem, answer,"对"};
							exam.AddOrUpdate(sql, str);
							System.out.println("第"+j+"题选：对");
						}
						else {//已存在，更改数据
				...
				RadioButton_F.addActionListener(new ActionListener(){
					public void actionPerformed(ActionEvent e){
						...
						if(!TrueOrFalseExamJframe.isExist(User.qbName+User.userID+String.valueOf(array[j-1]))) {//不存在，插入新数据
							String sql="insert into result (QuestionNo,QuestionID,QuestionStem,Answer,MyAnswer) values(?,?,?,?,?)";
							String[] str=new String[]{String.valueOf(j), User.qbName+User.userID+String.valueOf(array[j-1]), stem, answer,"错"};
							exam.AddOrUpdate(sql, str);
							System.out.println("第"+j+"题选：错");	
						}
						else {//已存在，更改数据
